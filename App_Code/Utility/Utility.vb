...
#Region "Many to Many Helpers"
	...
	Private Sub New()
	End Sub
    'Public Shared Function DeleteManyToManyRecord(ByVal mapTableName As String, ByVal column1 As String, ByVal column2 As String, ByVal column1Value As Integer, ByVal column2Value As Integer) As Integer
    '	'remove all records from the Map Table
    '	Dim delSql As String = "DELETE FROM " & mapTableName & " WHERE " & column1 & "=@c1 AND " & column2 & "=@c2"
    '	Dim cmd As QueryCommand = New QueryCommand(delSql)
    '	cmd.Parameters.Add("@c1", column1Value)
    '	cmd.Parameters.Add("@c2", column2Value)
    '	'execute it
    '	Dim iOut As Integer = DataService.ExecuteQuery(cmd)
    '	Return iOut
    'End Function
    '' <summary>
    '' Deletes records from mapping table where the value this object's primary key value
    ...
    '' <param name="mapTableName">The Mapping Table</param>
    '' <returns>Number of records deleted</returns>		
    'Public Shared Function SaveManyToManyRecord(ByVal mapTableName As String, ByVal column1 As String, ByVal column2 As String, ByVal column1Value As Integer, ByVal column2Value As Integer) As Integer
    '	'remove all records from the Map Table\
    '	'delete them first to be sure of no errors
    '	DeleteManyToManyRecord(mapTableName, column1, column2, column1Value, column2Value)

    '	Dim sql As String = "INSERT INTO " & mapTableName & " (" & column1 & "," & column2 & ") " & "VALUES (@c1, @c2)"
    ...
    '' <param name="mapTableName">The Mapping Table</param>
    '' <returns>Number of records deleted</returns>		
    'Public Shared Function DeleteManyToMany(ByVal pkColumnName As String, ByVal pkValue As Object, ByVal mapTableName As String, ByVal mapColumnName As String, ByVal mapColumnValue As Integer) As Integer

    '	Dim delSql As String = "DELETE FROM " & mapTableName & " WHERE " & pkColumnName & "=@pk AND " & mapColumnName & "=@map"
    '	Dim cmd As QueryCommand = New QueryCommand(delSql)
    ...
    '' <param name="mapTableName">The Mapping Table</param>
    '' <returns>Number of records deleted</returns>		
    'Public Shared Function DeleteAllManyToMany(ByVal pkColumnName As String, ByVal pkValue As Object, ByVal mapTableName As String) As Integer

    '	'remove all records from the Map Table
    '	Dim delSql As String = "DELETE FROM " & mapTableName & " WHERE " & pkColumnName & "=@pk"
    '	Dim cmd As QueryCommand = New QueryCommand(delSql)
    '	cmd.Parameters.Add("@pk", pkValue)
    ...
    'Public Shared Sub SaveManyToMany(ByVal pkColumnName As String, ByVal pkValue As Object, ByVal mapTableName As String, ByVal mapTableForiegnKey As String, ByVal items As ListItemCollection)
    '	'remove all existing 
    '	DeleteAllManyToMany(pkColumnName, pkValue, mapTableName)
    '	Dim cmd As QueryCommand = Nothing
...
#End Region
	...
	Public Shared Function GetUserName() As String
		...
		Return sUserName
    End Function
    'Public Shared Function makedate(ByVal givenDate As DateTime, ByVal givenEndDate As DateTime, ByVal formatType As formatType) As String
    '    Dim dayString As String = givenDate.Day.ToString
    ...
    '        Case formatType.DaystringMonthstringDayintegerTime
    '            Return Format(givenDate, "dddd") & ", " & Format(givenDate, "MMMM") & " " & dayString & ", " & Format(givenDate, "t") & " - " & Format(givenEndDate, "t")
    '        Case formatType.DayStringShortMonthstringShortDayIntegerTime
    '            Return Format(givenDate, "ddd") & ", " & Format(givenDate, "MMM") & " " & dayString & ", " & Format(givenDate, "t") & " - " & Format(givenEndDate, "t")
    '        Case formatType.DaystringMonthstringDayInteger
    '            Return Format(givenDate, "dddd") & ", " & Format(givenDate, "MMMM") & " " & dayString & ""
    '        Case formatType.Time
    '            Return Format(givenDate, "t") & " - " & Format(givenEndDate, "t")
    '        Case Else
    '            Return ""
    '    End Select
    'End Function

    Private Shared Function getFilename(ByVal pathInfo As String) As String
        Dim pageFilenameParts() As String = Split("/" & pathInfo, "/")
        Return pageFilenameParts(UBound(pageFilenameParts))
    End Function

    Public Shared Function cutOff(ByVal givenString As String, ByVal givenWidth As Integer) As String
...
#Region "CheckStringLength"
	...
	Public Shared Function CheckStringLength(ByVal stringToCheck As String, ByVal maxLength As Integer) As String
		...
		ElseIf stringToCheck.Length > 0 Then
			Dim words As String()
			Dim expectedWhitespace As Integer = stringToCheck.Length / 8
