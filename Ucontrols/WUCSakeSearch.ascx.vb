...
Imports System.Data
'Imports SslTools
Partial Class Ucontrols_WUCSakeSearch
            ...
            If Not Session("SearchParms") Is Nothing Then
                ...
                With hstQuery
                    ...
                    If .Item("SakeDiscriminatorSave") <> "" Then
                        ddlSakeDiscriminator.SelectedValue = RTrim(.Item("SakeDiscriminatorSave"))
                    End If
                    If .Item("Size") <> "" Then
                        ddlSize.SelectedValue = RTrim(.Item("Size"))
                    End If
                    If .Item("Region") <> "" Then
                        ddlPrefecture.SelectedValue = RTrim(.Item("Region"))
                    End If
                    If .Item("PolishingGrade") <> 0 Then
                        ddlPolishingGrade.SelectedValue = RTrim(.Item("PolishingGrade"))
                    End If
    ...
    Private Sub LoadUcombo()
        ...
        Dim _dvSakeRegion As New DataView
        Dim _dtSakeRegion As DataTable = Application("Region")
        _dvSakeRegion.Table = _dtSakeRegion
        _dvSakeRegion.RowFilter = "iLevel1Type = 3"
        '_dvSakeRegion.RowFilter = "cntryname = 'Japan' or cntryname = 'Korea' or regname = 'California'"
        ...
        ''load sake regionload region
        With ddlPrefecture

            .DataSource = _dvSakeRegion
            .DataMember = ""
        ...
        _dvSize.RowFilter = "iLevel1Type = 3"
        'load Size
        With ddlSize
            .DataSource = _dvSize
            .DataMember = ""
        ...
        _dvProducer.RowFilter = "iLevel1Type = 3"
        'load producer
        With ddlProducer
        ...
        'load SakeDiscriminator
        With ddlSakeDiscriminator
            .DataSource = Application("SakeDiscriminator")
            .DataMember = ""
            .DataValueField = "iSakeDiscriminator"
            .DataTextField = "sSakeDiscriminatorDesc"
            .DataBind()
        End With

        ' load sake polishing grade
        Dim _dtPolishingGrade As DataTable = Application("TypeCodes")
        Dim _dvPolishingGrade As New DataView
        _dvPolishingGrade.Table = _dtPolishingGrade
        _dvPolishingGrade.RowFilter = "sColumnName = 'iPolishing Grade'"
        With ddlPolishingGrade
            .DataSource = _dvPolishingGrade
            .DataMember = ""
    ...
    Private Sub Reset()
        ...
        ddlSakeDiscriminator.SelectedIndex = -1
        ddlSize.SelectedIndex = -1
        ddlPolishingGrade.SelectedIndex = -1
        ddlPrefecture.SelectedIndex = -1
    ...
    Protected Sub imbbSearch_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imbbSearch.Click
        ...
        updatesearch = Not chkOrganic.Checked
        ...
        Else
            iPriceRange = 0
        End If
        If ddlSakeDiscriminator.SelectedIndex <> 0 Then
            updatesearch = False
            sSakeDiscriminator = "(price.item in (select item from tblitemsakediscriminator where isakediscriminator = " & RTrim(ddlSakeDiscriminator.SelectedValue.ToString) & "))"
            sSakeDiscriminatorSave = RTrim(ddlSakeDiscriminator.SelectedValue.ToString)
        Else
            sSakeDiscriminator = String.Empty
            sSakeDiscriminatorSave = String.Empty
        End If
        ...
        Else
            sSize = String.Empty
        End If
        If ddlPrefecture.SelectedIndex <> 0 Then
            updatesearch = False
            sRegion = RTrim(ddlPrefecture.SelectedValue.ToString)
        Else
            sRegion = String.Empty
        End If

        If ddlPolishingGrade.SelectedIndex <> 0 Then
            updatesearch = False
            iPolishingGrade = Convert.ToInt16(ddlPolishingGrade.SelectedValue)
        Else
            iPolishingGrade = 0
        End If
        ...
        If sItemUPCLookup_value = "" And sNameLookup_value = "" And sCountry = "" And sRegion = "" And sSubRegion _
            = "" And sGrape = "" And sColor = "" And sVintageLow = "" And iPriceRange = 0 And sSakeDiscriminator = "" And sSize = "" _
            And iPolishingGrade = 0 And bOrganicall = 0 And sProducer = "" Then
            'Exit Sub
            'Redirect("SakeSearchNoResults.aspx?term=none", RedirectOptions.AbsoluteHttp)https_transfer_6/18/08
            Response.Redirect("SakeSearchNoResults.aspx?term=none")
        ...
        Else
            ...
            If dsLocal.Tables(0).Rows.Count = 0 Then
                ...
                'Redirect("SakeSearchNoResults.aspx?term=" & termQueryString, RedirectOptions.AbsoluteHttp)https_transfer_6/18/08
                Response.Redirect("~/SakeSearchNoResults.aspx?term=" & termQueryString & sbSearchQuery.ToString)
            ElseIf dsLocal.Tables(0).Rows.Count = 1 Then
                ...
                'Redirect("SearchResultsSingle.aspx?p=3&search=" + sSearchString + "&searchtype=Contains&term=" & termQueryString, RedirectOptions.AbsoluteHttp)https_transfer_6/18/08
                Response.Redirect("~/SearchResultsSingle.aspx?p=3&search=" + sSearchString + "&searchtype=Contains&term=" & termQueryString & sbSearchQuery.ToString)
            Else
                Session("DSMulti") = dsLocal
                Session("SearchParms") = hstQuery
                'Redirect("SakeSearchResult.aspx?p=3&search=Advanced&searchtype=Contains&term=" & termQueryString, RedirectOptions.AbsoluteHttp)https_transfer_6/18/08
                Response.Redirect("~/SakeSearchResult.aspx?p=3&search=Advanced&searchtype=Contains&term=" & termQueryString & sbSearchQuery.ToString)
            End If
        End If
    End Sub
